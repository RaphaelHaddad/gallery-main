    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        for (allowedModel in modelAllowlist.models) {
          ...
          // Also add image-supporting models to llm_server task
          if (model.llmSupportImage) {
            val serverTask = curTasks.find { it.id == "llm_server" }
            if (serverTask != null && !serverTask.models.contains(model)) {
              serverTask.models.add(model)
            }
          }
        }
